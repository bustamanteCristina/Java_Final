...
public class DrugStore {
        ...
        while (true) {
            ...
            try {
                ...
                if (u_age >= 18) {
                    ...
                    System.err.println("18 BELOW ARE NOT ALLOWED TO REGISTER!");
                }
    ...
    public void addStock() {
        medicines.add(new Medicine(100, "benzonatate", "Tessalon", "cough", 100));
        medicines.add(new Medicine(100, "guaifenesin", "Robitussin Mucus", "cough", 100));
        medicines.add(new Medicine(100, "codeine", "Allfen CD", "cough", 100));
        medicines.add(new Medicine(100, "caffeine", "capacet", "headache", 100));
        medicines.add(new Medicine(100, "Ibuprofen", "Midol", "headache", 100));
        medicines.add(new Medicine(100, "\tAdvil", "\tIbuprofen", "headache", 100));
        medicines.add(new Medicine(100, "naproxen", "Aleve", "body pain", 100));
        medicines.add(new Medicine(100, "Acetminophen", "Actamin", "body pain", 100));
        medicines.add(new Medicine(100, "Declofenac", "Cambia", "body pain", 100));
        medicines.add(new Medicine(100, "Hydroxyzine", "Vistaril", "alergy", 100));
        medicines.add(new Medicine(100, "Levocetiricine", "Xyzal", "alergy", 100));
        medicines.add(new Medicine(100, "Doxylamine", "Usinom", "alergy", 100));
    }

    public void displayMed(String type) {
        printLine();
        System.out.printf("%-20s %-20s %-20s %-20s %-20s %-20s\n", "BRAND", "GENERIC NAME", "TYPE", "PRICE", "QUANITY");
                ...
                if (a instanceof Customer) {
                    if (!((Customer) a).getPurchasedMed().isEmpty()) {
                        for (Medicine m2 : ((Customer) a).getPurchasedMed()) {
                            if (m.getGenericName().equals(m2.getGenericName())) {
    ...
    public Medicine inputMedicine() {
        ...
        return new Medicine(price, genName, brandName, type, quantity);
    }

    public void removeMedicine() {
        String medName = stringInput("\nEnter medicine name: ");
        if (!"Medicine not found".equals(searchMedId(medName))) {
        ...
        while (true) {
            ...
            if (searchMed(name).getType() != null || searchMed(name).getQuantity() != 0) {
                ...
                if (m.getQuantity() >= quantity) {
                    ...
                    Medicine med = new Medicine(m.getGenericName(), m.getPrice(), quantity);
                    a.addPurchased(med);
                    pm.add(med);
                } else {
            ...
            if (answer.equalsIgnoreCase("n")) {
                computePurchased_Total(pm, a.getAge());
                break;
            }
        }
    }
    ...
    public void payment(double total) {
        int amount = this.intInput("Amount : ");
    ...
    public void computePurchased_Total(ArrayList<Medicine> m, int age) {
        if (!m.isEmpty()) {
            System.out.println("\n\n_________________________________________");
            double total = 0;
            for (Medicine n : m) {
    ...
    public Medicine searchMed(String name) {
        for (Medicine m : medicines) {
            if (m.getGenericName().equalsIgnoreCase(name)) {
                return m;
            }
        }
        return new Medicine();
    ...
    public String searchMedId(String name) {
        for (int i = 0; i < medicines.size(); ++i) {
            if (medicines.get(i).getGenericName().equalsIgnoreCase(name)) {
                return i + "";
            }
        }
